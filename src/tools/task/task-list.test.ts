...
describe("createTaskList", () => {
  ...
  beforeEach(() => {
    taskDir = join(testProjectDir, ".sisyphus-light/tasks")
  ...
  it("returns empty array when no tasks exist", async () => {
    ...
    const config = {
      "sisyphus-light": {
        tasks: {
          storage_path: join(testProjectDir, ".sisyphus-light/tasks"),
  ...
  it("excludes completed tasks by default", async () => {
    ...
    writeJsonAtomic(join(testProjectDir, ".sisyphus-light/tasks", "T-1.json"), task1)
    writeJsonAtomic(join(testProjectDir, ".sisyphus-light/tasks", "T-2.json"), task2)
  ...
  it("excludes deleted tasks by default", async () => {
   ...
   it("returns summary format with id, subject, status, owner, blockedBy", async () => {
    ...
    const task: TaskObject = {
      ...
      status: "in_progress",
      owner: "sisyphus-light",
      blocks: [],
      blockedBy: ["T-2"],
      threadID: "test-session",
    }

    writeJsonAtomic(join(testProjectDir, ".sisyphus-light/tasks", "T-1.json"), task)
    ...
    expect(summary.owner).toBe("sisyphus-light")
  ...
  it("filters blockedBy to only include unresolved (non-completed) blockers", async () => {
    ...
    writeJsonAtomic(join(testProjectDir, ".sisyphus-light/tasks", "T-blocker-completed.json"), blockerCompleted)
    writeJsonAtomic(join(testProjectDir, ".sisyphus-light/tasks", "T-blocker-pending.json"), blockerPending)
    writeJsonAtomic(join(testProjectDir, ".sisyphus-light/tasks", "T-main.json"), mainTask)
