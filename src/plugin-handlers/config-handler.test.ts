...
import * as sisyphusJunior from "../agents/sisyphus-light-junior"
...
describe("Sisyphus-Junior model inheritance", () => {
  test("does not inherit UI-selected model as system default", async () => {
    ...
    expect(agentConfig[getAgentDisplayName("sisyphus-light-junior")]?.model).toBe(
      sisyphusJunior.SISYPHUS_JUNIOR_DEFAULTS.model
    )
  })

  test("uses explicitly configured sisyphus-light-junior model", async () => {
    // #given
    const pluginConfig: OhMyOpenCodeConfig = {
      agents: {
...
describe("Plan agent demote behavior", () => {
  test("orders core agents as sisyphus-light -> hephaestus-light -> prometheus-light -> atlas-light", async () => {
    // #given
    ...
    const coreAgents = [
      ...
      getAgentDisplayName("hephaestus-light"),
      getAgentDisplayName("prometheus-light"),
      getAgentDisplayName("atlas-light"),
    ]
    const ordered = keys.filter((key) => coreAgents.includes(key))
    expect(ordered).toEqual(coreAgents)
  })

  test("plan agent should be demoted to subagent without inheriting prometheus-light prompt", async () => {
    // #given
    const pluginConfig: OhMyOpenCodeConfig = {
      sisyphus_agent: {
        planner_enabled: true,
    ...
    // #then - plan is demoted to subagent but does NOT inherit prometheus-light prompt
    ...
    expect(agents[getAgentDisplayName("prometheus-light")]?.prompt).toBeDefined()
  })
  ...
  test("plan agent remains unchanged when planner is disabled", async () => {
    ...
    // #then - plan is not touched, prometheus-light is not created
    const agents = config.agent as Record<string, { mode?: string; name?: string; prompt?: string }>
    expect(agents[getAgentDisplayName("prometheus-light")]).toBeUndefined()
    expect(agents.plan).toBeDefined()
    expect(agents.plan.mode).toBe("primary")
    expect(agents.plan.prompt).toBe("original plan prompt")
  })

  test("prometheus-light should have mode 'all' to be callable via task", async () => {
    // given
    const pluginConfig: OhMyOpenCodeConfig = {
      sisyphus_agent: {
        planner_enabled: true,
    ...
    const prometheusKey = getAgentDisplayName("prometheus-light")
...
describe("Agent permission defaults", () => {
  test("hephaestus-light should allow task", async () => {
    // #given
    const createBuiltinAgentsMock = agents.createBuiltinAgents as unknown as {
      mockResolvedValue: (value: Record<string, unknown>) => void
    }
    ...
    const hephaestusKey = getAgentDisplayName("hephaestus-light")
...
describe("default_agent behavior with Sisyphus orchestration", () => {
  test("canonicalizes configured default_agent with surrounding whitespace", async () => {
    ...
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
      default_agent: "  hephaestus-light  ",
      agent: {},
    }
    const handler = createConfigHandler({
      ctx: { directory: "/tmp" },
    ...
    // then
    expect(config.default_agent).toBe(getAgentDisplayName("hephaestus-light"))
  })
  ...
  test("preserves existing display-name default_agent", async () => {
    // #given
    const pluginConfig: OhMyOpenCodeConfig = {}
    const displayName = getAgentDisplayName("hephaestus-light")
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
  ...
  test("sets default_agent to sisyphus-light when missing", async () => {
    // #given
    const pluginConfig: OhMyOpenCodeConfig = {}
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
    ...
    // #then
    expect(config.default_agent).toBe(getAgentDisplayName("sisyphus-light"))
  })

  test("sets default_agent to sisyphus-light when configured default_agent is empty after trim", async () => {
    // given
    const pluginConfig: OhMyOpenCodeConfig = {}
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
...
describe("Prometheus direct override priority over category", () => {
  test("direct reasoningEffort takes priority over category reasoningEffort", async () => {
    ...
    const pKey = getAgentDisplayName("prometheus-light")
  ...
  test("prometheus-light prompt_append is appended to base prompt", async () => {
    // #given - prometheus-light override with prompt_append
...
describe("Plan agent model inheritance from prometheus-light", () => {
  test("plan agent inherits all model-related settings from resolved prometheus-light config", async () => {
    //#given - prometheus-light resolves to claude-opus-4-6 with model settings
    ...
    //#then - plan inherits model and variant from prometheus-light, but NOT prompt
  ...
  test("plan agent inherits temperature, reasoningEffort, and other model settings from prometheus-light", async () => {
    //#given - prometheus-light configured with category that has temperature and reasoningEffort
    spyOn(shared, "resolveModelPipeline" as any).mockReturnValue({
      model: "openai/gpt-5.4",
      provenance: "override",
      variant: "high",
    ...
    //#then - plan inherits ALL model-related settings from resolved prometheus-light
  ...
  test("plan agent user override takes priority over prometheus-light inherited settings", async () => {
    //#given - prometheus-light resolves to opus, but user has plan override for gpt-5.4
    spyOn(shared, "resolveModelPipeline" as any).mockReturnValue({
      model: "anthropic/claude-opus-4-6",
      provenance: "provider-fallback",
      variant: "max",
    ...
    //#then - plan uses its own override, not prometheus-light settings
    const agents = config.agent as Record<string, Record<string, unknown>>
    expect(agents.plan.model).toBe("openai/gpt-5.4")
    expect(agents.plan.variant).toBe("high")
  ...
  test("plan agent does NOT inherit prompt, description, or color from prometheus-light", async () => {
    //#given
    spyOn(shared, "resolveModelPipeline" as any).mockReturnValue({
      model: "anthropic/claude-opus-4-6",
      provenance: "provider-fallback",
...
describe("per-agent todowrite/todoread deny when task_system enabled", () => {
  const AGENTS_WITH_TODO_DENY = new Set([
    ...
    getAgentDisplayName("atlas-light"),
    getAgentDisplayName("prometheus-light"),
    getAgentDisplayName("sisyphus-light-junior"),
  ])
  ...
  test("does not deny todowrite/todoread when task_system is disabled", async () => {
    ...
    //#then
    const agentResult = config.agent as Record<string, { permission?: Record<string, unknown> }>
    expect(agentResult[getAgentDisplayName("sisyphus-light")]?.permission?.todowrite).toBeUndefined()
    expect(agentResult[getAgentDisplayName("sisyphus-light")]?.permission?.todoread).toBeUndefined()
    expect(agentResult[getAgentDisplayName("hephaestus-light")]?.permission?.todowrite).toBeUndefined()
    expect(agentResult[getAgentDisplayName("hephaestus-light")]?.permission?.todoread).toBeUndefined()
  })
