    ...
    "tool.execute.before": async (
      ...
      if (!filePath) {
       ...
       if (!isAllowedFile(filePath, ctx.directory)) {
         log(`[${HOOK_NAME}] Blocked: Prometheus can only write to .sisyphus-light/*.md`, {
           sessionID: input.sessionID,
           tool: toolName,
           filePath,
           agent: agentName,
         ...
         throw new Error(
           `[${HOOK_NAME}] ${getAgentDisplayName("prometheus-light")} can only write/edit .md files inside .sisyphus-light/ directory. ` +
           `Attempted to modify: ${filePath}. ` +
           `${getAgentDisplayName("prometheus-light")} is a READ-ONLY planner. Use /start-work to execute the plan. ` +
           `APOLOGIZE TO THE USER, REMIND OF YOUR PLAN WRITING PROCESSES, TELL USER WHAT YOU WILL GOING TO DO AS THE PROCESS, WRITE THE PLAN`
         )
       }

      const normalizedPath = filePath.toLowerCase().replace(/\\/g, "/")
      if (normalizedPath.includes(".sisyphus-light/plans/") || normalizedPath.includes(".sisyphus-light\\plans\\")) {
      ...
      log(`[${HOOK_NAME}] Allowed: .sisyphus-light/*.md write permitted`, {
        sessionID: input.sessionID,
        tool: toolName,
        filePath,
        agent: agentName,
