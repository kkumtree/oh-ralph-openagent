...
describe("look_at tool conditional registration", () => {
  describe("isMultimodalLookerEnabled logic", () => {
    // given multimodal-looker-light is in disabled_agents
    // when checking if agent is enabled
    // then should return false (disabled)
    it("returns false when multimodal-looker-light is disabled (exact case)", () => {
      const disabledAgents: string[] = ["multimodal-looker-light"]
    ...
    // given multimodal-looker-light is in disabled_agents with different case
    // when checking if agent is enabled
    // then should return false (case-insensitive match)
    it("returns false when multimodal-looker-light is disabled (case-insensitive)", () => {
    ...
    // given multimodal-looker-light is NOT in disabled_agents
    // when checking if agent is enabled
    // then should return true (enabled)
    it("returns true when multimodal-looker-light is not disabled", () => {
      const disabledAgents: string[] = ["oracle-light", "librarian-light"]
      const isEnabled = !disabledAgents.some(
        (agent) => agent.toLowerCase() === "multimodal-looker-light"
      )
