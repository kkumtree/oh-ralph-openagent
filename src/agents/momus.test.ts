...
import { MOMUS_SYSTEM_PROMPT } from "./momus-light"
...
describe("MOMUS_SYSTEM_PROMPT policy requirements", () => {
  ...
  test("should extract paths containing .sisyphus-light/plans/ and ending in .md", () => {
    // given
    const prompt = MOMUS_SYSTEM_PROMPT

    // when / #then
    expect(prompt).toContain(".sisyphus-light/plans/")
  ...
  test("should NOT teach that 'Please review' is INVALID (conversational wrapper allowed)", () => {
    ...
    const invalidExample = "Please review .sisyphus-light/plans/plan.md"
