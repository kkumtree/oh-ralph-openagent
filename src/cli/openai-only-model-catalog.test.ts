...
describe("generateModelConfig OpenAI-only model catalog", () => {
  test("fills remaining OpenAI-only agent gaps with OpenAI models", () => {
    ...
    // #then
    expect(result.agents?.explore-light).toEqual({ model: "openai/gpt-5.4", variant: "medium" })
    expect(result.agents?.librarian-light).toEqual({ model: "openai/gpt-5.4", variant: "medium" })
  })
